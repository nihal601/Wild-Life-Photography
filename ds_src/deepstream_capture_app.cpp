#include "wildlifephography.h"

namespace WildLifePhotography {
  void
  Capture::update_fps(gint id) {
  ...
  int
  Capture::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  Capture::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  Capture::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
  ...
  GstPadProbeReturn
  Capture::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      ...
      for (l_obj = frame_meta->obj_meta_list; l_obj != NULL;
        ...
        if(class_index == Tiger) {
          changeBBoxColor(obj_meta, 1, 1.0, 0.0, 0.0, 0.25);
          cout << "Camera " << frame_meta->source_id << " Captured Tiger" <<  endl;
        }
        if(class_index == Gazelle) {
          changeBBoxColor(obj_meta, 1, 0.0, 1.0, 0.0, 0.25);
          cout << "Camera " << frame_meta->source_id << " Captured Gazelle" <<  endl;
        }
        if(class_index == Flamingos) {
          changeBBoxColor(obj_meta, 1, 0.0, 0.0, 1.0, 0.25);
          cout << "Camera " << frame_meta->source_id << " Captured Flamingos" <<  endl;
        }
        if(class_index == Kingfisher) {
          changeBBoxColor(obj_meta, 1, 1.0, 1.0, 0.0, 0.25);
          cout << "Camera " << frame_meta->source_id << " Captured Kingfisher" <<  endl;
        }
  ...
  gboolean
  Capture::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  Capture::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  Capture::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  Capture::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  Capture::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  Capture::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  Capture::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        WildLifePhotography::Capture::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", WildLifePhotography::Capture::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!WildLifePhotography::Capture::set_tracker_properties(nvtracker)) {
  ...
  void Capture::setPaths(guint num_sources) {

    // Config Paths
    PGIE_YOLO_DETECTOR_CONFIG_FILE_PATH =
    strdup("models/YOLOv3Wild/config_infer_primary_yolov3.txt");
...
int main(int argc, char *argv[]) {
  ...
  WildLifePhotography::Capture capture;
  GMainLoop *loop = NULL;
  ...
  ctx = g_option_context_new("Capture DeepStream App");
  group = g_option_group_new("Capture", NULL, NULL, NULL, NULL);
  g_option_group_add_entries(group, capture.entries);
  ...
  gint sources = capture.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  int fail_safe = capture.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, capture.bus_call, loop);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      capture.tiler_src_pad_buffer_probe, NULL, NULL);
  }
